...
- `setContactField(field: keyof IOrderForm, value: string): void` - Устанавливает контактные данные (email или phone) в order.
...
### Класс `Catalog`
`constructor(container: HTMLElement, createCard: (data: ICardData) => Card)` - Инициализация контейнера для карточек и функции создания карточек.
**Назначение:** 
...
- `_items:Array<ICardData>`	- Список данных товаров для отображения.
- `_createCard	(data: ICardData) => Card` - функция, принимающая данные товара и возвращающая готовый компонент Card.

**Методы:**
- `render(items: ICardData[]): void` -	Очищает каталог и отрисовывает список карточек.
- `addItem(data: ICardData): void`	- создает и добавляет одну карточку в контейнер.
...
- `_image: HTMLElement` - элемент изображения товара.
- `_description: HTMLElement` - элемент описания товара.
- `_button: HTMLElement` - элемент кнопки ("Добавить в корзину" или "В корзине")
- `_category: HTMLElement` - элемент категории товара.
- `_price: HTMLElement` - элемент отображения цены товара.
- `_handleAddToCart: (item: IProductItem) => void` — сохранённый колбэк из конструктора.

**Методы:**
- `render(): HTMLElement` - Создает и возвращает готовую карточку товара как DOM-элемент.
- `setEventListeners(): void` - навешивает обработчики событий (например, клик по кнопке "Добавить в корзину").
- `update(data: IProductItem): void` - обновляет содержимое карточки при изменении данных товара (например, цена, наличие), без пересоздания элемента.
...
- `_items: IProductItem[]` — список товаров, добавленных в корзину.
- `_createBasketItem: (data: IProductItem) => HTMLElement` — функция-фабрика для создания DOM-элемента товара в корзине.
...
- `getItems(): IProductItem[]` — возвращает текущий список товаров в корзине.
...
- `content: HTMLElement` - Свойство для установки нового содержимого модального окна. Заменяет текущее содержимое элементами из переданного параметра.
...
**Методы:**
- `render(totalAmount: number, message: string): void` - Отображает успешное сообщение, устанавливает текстовое сообщение и сумму списания.
...
- `email: string` — текущее значение email из поля формы.
- `phone: string` — текущее значение телефона из поля формы.
...
- `validate(): boolean` - Выполняет валидацию введённых данных (например, проверяет корректность формата телефона и почты).
...
- `validate(): boolean` — проверяет, что и `payment`, и `address` непустые; возвращает `true`, если корректны.
- `setPaymentMethod(method: string): void` — устанавливает `this.payment = method` и обновляет стиль кнопок.
- `handleSubmit(event: SubmitEvent): void` — отменяет стандартную отправку формы, вызывает `validate()`, а затем `submitOrder()` при успешной проверке.
